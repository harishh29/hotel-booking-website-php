<?php 
    ...
    if(isset($_POST['add_room'])){
        ...
        $q1 = "INSERT INTO `rooms`(`room_name`, `area`, `price`, `quantity`, `adult`, `children`, `description`) VALUES (?,?,?,?,?,?,?)";
        $values = [$frm_data['room_name'], $frm_data['area'], $frm_data['price'], $frm_data['quantity'], $frm_data['adult'], $frm_data['children'],$frm_data['description']];
        ...
        while($row = mysqli_fetch_assoc($res)){
            ...
            <tr class='align-middle'>
                ...
                <td>
                    ...
                    <button type='button' onclick=\"room_images($row[id], '$row[room_name]')\" class='btn btn-info shadow-none btn-sm' data-bs-toggle='modal' data-bs-target='#room-images'>
    ...
    if(isset($_POST['edit_room'])){
        ...
        $q1 = "UPDATE `rooms` SET `room_name`=?,
                `area`=?,`price`=?,`quantity`=?,
                `adult`=?,`children`=?,`description`=? WHERE `id`=?";
        $values = [$frm_data['room_name'], $frm_data['area'], $frm_data['price'], $frm_data['quantity'], $frm_data['adult'], $frm_data['children'],$frm_data['description'],$frm_data['room_id']];
